const templateBase = `${window.MOCKUP_DEV_BASE || "assets-mockup"}/templates`;
...
async function injectTemplates() {
  ...
  // 4) Evento global para que app.js haga boot cuando TODO esté listo
...
injectTemplates().catch((err) => {
  ...
  // Fallback visual mínimo para debug rápido
  const root = document.getElementById("templateRoot");
  if (root) {
    root.innerHTML = `
