...
class GatewaySuscriptionRepository implements GatewaySuscriptionRepositoryInterface, ActualizaEstadoInterface
{
    private PDO $pdo;
    ...
    public function create(array $data): int
    {
        $stmt = $this->pdo->prepare(
            'INSERT INTO gateway_suscription
    ...
    {
        $stmt = $this->pdo->prepare('UPDATE gateway_suscription SET estado = :estado WHERE id = :id');
        $stmt->execute(['estado' => $estado, 'id' => $id]);
    }

    public function findByRequestId(string $requestId): ?array
    {
        $stmt = $this->pdo->prepare('SELECT * FROM gateway_suscription WHERE request_id = :request_id LIMIT 1');
    ...
    {
        $stmt = $this->pdo->prepare(
            'SELECT * FROM gateway_suscription WHERE correo = :correo ORDER BY created_at DESC'
        );
