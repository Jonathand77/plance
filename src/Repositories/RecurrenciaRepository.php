...
class RecurrenciaRepository implements RecurrenciaRepositoryInterface, ActualizaEstadoInterface
{
    private PDO $pdo;
    ...
    {
        $stmt = $this->pdo->prepare('UPDATE recurrencias SET request_id = :request_id WHERE id = :id');
        $stmt->execute(['request_id' => $requestId, 'id' => $id]);
    }

    public function updateEstado(int $id, string $estado): void
    {
        $stmt = $this->pdo->prepare('UPDATE recurrencias SET estado = :estado WHERE id = :id');
    ...
    public function updateEstadoYFechaFin(int $id, string $estado, ?string $fechaFin): void
    {
        ...
        $stmt = $this->pdo->prepare(
            'UPDATE recurrencias SET estado = :estado, fecha_fin = :fecha_fin WHERE id = :id'
        );
    ...
    {
        $stmt = $this->pdo->prepare('SELECT * FROM recurrencias WHERE id = :id LIMIT 1');
        $stmt->execute(['id' => $id]);
        $row = $stmt->fetch();
    ...
    {
        $stmt = $this->pdo->prepare('SELECT * FROM recurrencias WHERE request_id = :request_id LIMIT 1');
    ...
    {
        $stmt = $this->pdo->prepare(
            'SELECT * FROM recurrencias WHERE usuario_id = :usuario_id ORDER BY created_at DESC'
        );
    ...
    {
        $stmt = $this->pdo->prepare(
            'SELECT * FROM recurrencias WHERE id = :id AND usuario_id = :usuario_id LIMIT 1'
        );
    ...
    {
        $stmt = $this->pdo->prepare(
            "SELECT id, CONCAT(servicio, ' — ', plan) AS nombre, precio, usuario_id AS usuario,
                    created_at, estado, 'recurrencia' AS tipo
             FROM recurrencias WHERE estado = 'aprobada' AND usuario_id = :usuario_id
    ...
    {
        $stmt = $this->pdo->prepare(
            "SELECT *, 'recurrencia' AS tipo FROM recurrencias
